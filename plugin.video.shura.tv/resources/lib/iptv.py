...
LASTLISTFILE = os.path.join(xbmcvfs.translatePath(
    'special://temp/'), 'last.ShuraTVPLaylist')
LASTEPGFILE = os.path.join(xbmcvfs.translatePath(
    'special://temp/'), 'last.ShuraTVPEPG')
try:
    import json
except ImportError:
...
class shura:
    ...
    def __init__(self, OTT, StreamType, ServerName, addonid=''):
        ...
        if os.path.isfile(LASTEPGFILE):
            l = open(LASTEPGFILE, 'r', encoding='utf-8')
            try:
    ...
    def _errors_check(self, json):
        ...
        if 'error' in json:
            ...
            if 'message' in err:
                ...
                if message == None:
                    message = err['code']
                xbmc.log('[SHURA.TV] ERROR: %s' % message)
    ...
    def getLast(self):
        ...
        if not self.last_list:
            if os.path.isfile(LASTLISTFILE):
                f = open(LASTLISTFILE, 'r', encoding='utf-8')
                try:
    ...
    def getChannelsList(self):
        ...
        for feed in dom.firstChild.childNodes:
            ...
            if feed.nodeName == 'feed':
                # No longer have valid id, it is empty currently
                id = feed.getAttribute('id')
                name = feed.getElementsByTagName(
                    'name')[0].firstChild.wholeText
                if self.StreamType == '1':
                    #xbmc.log('url type=HLS')
                    url = feed.getElementsByTagName(
                        'url_hls')[0].firstChild.data
                else:
        ...
        self.last_list = {'channels': res, 'ttl': now.strftime(
            '%Y-%m-%d'), 'ott': self.OTT, 'server': self.ServerName, 'streamType': self.StreamType}
        f = open(LASTLISTFILE, 'w', encoding='utf-8')
        try:
            jsave = JSONENCODE(self.last_list)
            f.write(jsave)
        except Exception as e:
    ...
    def haveEPG(self, FEED):
	    return str(int(FEED)) in self.last_epg
    ...
    def getCurrentEPG(self, HOST, FEED):
        ...
        postparams = None
        try:
            req = urllib.request.Request(url, postparams, {'User-agent': ua, 'Connection': 'Close', 'Accept': 'application/json, text/javascript, */*',
                                         'X-Requested-With': 'XMLHttpRequest', 'Content-Type': 'text/html;charset=utf-8'})
            xbmc.log('[SHURA.TV] get current epg url=' + str(url))
            resstream = urllib.request.urlopen(req)
            resp = resstream.read()
        ...
        try:
            ...
            xbmc.log('[SHURA.TV] Error in decoding json currentEPG' +
                     str(e) + 'for'+str(resp) + 'len=' + str(len(resp)))
            resp2 = None
        else:
            resstream.close()

        self.last_epg[str(int(FEED))] = resp2
        g = open(LASTEPGFILE, 'w', encoding='utf-8')
        try:
    ...
    def getLastEPG(self, HOST, FEED):
        ...
        if len(self.last_epg) == 0:
            ...
            if os.path.isfile(LASTEPGFILE):
                f2 = open(LASTEPGFILE, 'r', encoding='utf-8')
                try:
                    self.last_epg = JSONDECODE(f2.read)
                except Exception as e:
                    xbmc.log('[SHURA.TV] Error loading last epg %s' % e)
